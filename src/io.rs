...
impl Io {
    ...
    pub fn receive_event<T>(
        ...
        self.touch.receive_event(event);
    }
}

#[derive(Default)]
pub struct Touch {
    pub start_position: Option<nalgebra_glm::Vec2>,
    pub current_position: nalgebra_glm::Vec2,
    pub touch_delta: nalgebra_glm::Vec2,
    pub moved: bool,
}

impl Touch {
    pub fn receive_event<T>(&mut self, event: &winit::event::Event<T>) {
        if let winit::event::Event::NewEvents { .. } = event {
            if !self.moved {
                self.touch_delta = nalgebra_glm::vec2(0.0, 0.0);
            }
            self.moved = false;
        }
        ...
        } = *event
        {
            match phase {
                winit::event::TouchPhase::Started => {
                    let location = nalgebra_glm::vec2(location.x as f32, location.y as f32);
                    self.start_position = Some(location)
                }
                ...
                winit::event::TouchPhase::Moved => {
                    ...
                    self.touch_delta = match self.current_position - self.start_position.unwrap() {
                        delta if delta.x.abs() > 0.0 || delta.y.abs() > 0.0 => delta,
                        _ => nalgebra_glm::vec2(0.0, 0.0),
                    };
                    self.moved = true;
                }
                _ => {}
