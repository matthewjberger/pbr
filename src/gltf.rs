pub fn import_gltf_file(path: impl AsRef<std::path::Path>) -> crate::world::World {
    let (gltf, buffers, raw_images) = gltf::import(path).expect("Failed to import gltf");
    import_gltf_raw(gltf, buffers, raw_images)
}
                    ...
                    primitives: mesh
                        ...
                        .map(|primitive| {
                            let primitive_vertices: Vec<crate::world::Vertex> = {
                                ...
                                let map_to_vec2 =
                            |coords: gltf::mesh::util::ReadTexCoords<'_>| -> Vec<nalgebra_glm::Vec2> {
                                coords
                                    .into_f32()
                                ...
                                let convert_joints =
                                |joints: gltf::mesh::util::ReadJoints<'_>| -> Vec<nalgebra_glm::Vec4> {
                                    joints
                                        .into_u16()
                                        .map(|joint| {
                                            nalgebra_glm::vec4(
                                ...
                                let convert_weights =
                            |weights: gltf::mesh::util::ReadWeights<'_>| -> Vec<nalgebra_glm::Vec4> {
                                ...
                                let convert_colors =
                                |colors: gltf::mesh::util::ReadColors<'_>| -> Vec<nalgebra_glm::Vec3> {
                                    colors
                                        .into_rgb_f32()
...
impl From<gltf::Camera<'_>> for crate::world::Camera {
    fn from(camera: gltf::Camera<'_>) -> Self {
        Self {
